...
import com.android.wallpaper.util.Cam
import com.android.wallpaper.util.CamUtils
...
class ColorScheme(
    ...
    val fgColors = arrayListOf <ColorCustomOption.ColorCustomInfo>()
    val bgColors = arrayListOf <ColorCustomOption.ColorCustomInfo>()
    ...
    @JvmOverloads
    constructor(
        wallpaperColors: WallpaperColorWrap,
        darkTheme: Boolean,
        style: Style = Style.TONAL_SPOT
    ):
            this(getSeedColor(wallpaperColors, style != Style.CONTENT), darkTheme, style)
...
//        get() = ColorUtils.setAlphaComponent(if (darkTheme) neutral1[8] else neutral1[0], 0xFF)
        get() = ColorUtils.setAlphaComponent(if (darkTheme) neutral1[8] else neutral1[1], 0xFF)
    val accentColor
    ...
    companion object {
        ...
        fun getWallpaperColorAllColors(wallpaperColors :WallpaperColors):Map<Int, Int> {
            Log.i("测试", "getWallpaperColorAllColors: ")
            if(WallpaperColorWrap.mHomeWallpaperColorsWrap != null
                && wallpaperColors.primaryColor ==
                WallpaperColorWrap.mHomeWallpaperColorsWrap.mainColors[0]){
                return WallpaperColorWrap.mHomeWallpaperColorsWrap.allColors
            }
            if(WallpaperColorWrap.mLockWallpaperColorsWrap != null
                && wallpaperColors.primaryColor ==
                WallpaperColorWrap.mLockWallpaperColorsWrap.mainColors[0]){
                return WallpaperColorWrap.mLockWallpaperColorsWrap.allColors
            }
            return HashMap()
        }

        fun getWallpaperColorMainColor(wallpaperColors :WallpaperColors):List<Color>{
            Log.i("测试", "getWallpaperColorMainColor: ")
            if(WallpaperColorWrap.mHomeWallpaperColorsWrap != null
                && wallpaperColors.primaryColor ==
                WallpaperColorWrap.mHomeWallpaperColorsWrap.mainColors[0]){
                return WallpaperColorWrap.mHomeWallpaperColorsWrap.mainColors
            }
            if(WallpaperColorWrap.mLockWallpaperColorsWrap != null
                && wallpaperColors.primaryColor ==
                WallpaperColorWrap.mLockWallpaperColorsWrap.mainColors[0]){
                return WallpaperColorWrap.mLockWallpaperColorsWrap.mainColors
            }
            return ArrayList()
        }
        /**
        ...
        fun getSeedColors(wallpaperColors: WallpaperColors, filter: Boolean = true): List<Int> {
            val allColors = getWallpaperColorAllColors(wallpaperColors)
            val mainColors = getWallpaperColorMainColor(wallpaperColors)
            val totalPopulation = if (allColors.values.isEmpty()) 0.0 else allColors.values.reduce { a, b -> a + b }
                .toDouble()
            ...
            if (totalPopulationMeaningless) {
                ...
                val distinctColors = mainColors.map {
                    it.toArgb()
                }.distinct().filter {
                    if (!filter) {
                        true
            ...
            val intToProportion = allColors.mapValues {
            ...
            val intToHueProportion = allColors.mapValues {
        ...
        @JvmOverloads
        @ColorInt
        fun getSeedColor(wallpaperColors: WallpaperColorWrap, filter: Boolean = true): Int {
            return getSeedColors(wallpaperColors, filter).first()
        }
        @JvmStatic
        @JvmOverloads
        fun getSeedColors(wallpaperColors: WallpaperColorWrap, filter: Boolean = true): List<Int> {
            val allColors = wallpaperColors.allColors
            val mainColors = wallpaperColors.mainColors
            val mainColorsInt = wallpaperColors.mainColorsInt;
            val totalPopulation = if (allColors.values.isEmpty()) 0.0 else allColors.values.reduce { a, b -> a + b }
                .toDouble()
            val totalPopulationMeaningless = (totalPopulation == 0.0)
            if (totalPopulationMeaningless) {
                ...
                //
                // In this case, the colors are usually from a Live Wallpaper.
                val distinctColors = if (BuildCompatUtils.isAtLeastO()) {
                    mainColors.map {
                        it.toArgb()
                    }.distinct().filter {
                        if (!filter) {
